...
The answer to this Q (and to the related question of if state is Causal) **cannot come from just observing this data**
**i.e. there's nothing in the data or expriment that says that chaning states for a senator would make more bills, so cannot say if state is causal**
...
**We can change the term limits and observe performance. In a way, that's what the experiment does within each state. Terms could be causal**
...
# Two tailed p value
p_two_arkansas <- mean(abs(distribution.under.sharp.null.texas) >= abs(arkansas_ate))
p_two_arkansas
```
**Answer: p value for ATE for Arkansas is 0.3162**
**p value is not less than .05. We cannot reject the SHARP NULL hypothesis**
