...
**Answer: No, "has cell phone" is not bad control. It is not an outcome of the treatment.**
**From the regression it looks fairly inconsequential (coefficient is .022(.014)). It may be capturing some of the variance and effect that would otherwise show up in  "Any SMS message" variable (SMS would have no impact if you do not have a cell phone. That commonsense observation is also mathematically captured by this variable).**
...
**Answer: The coefficient of "Any SMS message" would go up if we remove "has cell phone" form the regression. 4A shows a positive impact of both an sms message and having a cell phone. In absence of the "has cell phone" variable, its impact would be mostly captured by the "sms" variable, driving up its magnitude.**
...
2. SMS (3 levels: No message, generic message, specific message)
3. Cell phone (2 levels: has, has not)  
**Answer: The dimensions could be written as 3 * 3 * 2**

*The following were also consequential for the experiment:*
1. Baselines for each of the dependent variable
...
hist(d4$street)
```
**So yes, just a couple of troubling values (-999) in "street". We can replace them with NA**

```{r}
d4$street[d4$street == -999] <- NA
hist(d4$street)
```
*Good distribution among the streets now*
